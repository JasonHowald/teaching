...
:::{#def-ring}
### Ring
A (unital) **ring** is a set $R$ together with four structural elements: A chosen element called $0_R$, a chosen element called $1_R$, an operation called addition, written $+_R$, and an operation called multiplication, written $\cdot_R$ or $\times_R$ (usually unwritten, expressed as implied product), satisfying the following **ring axioms**:
...
The binomial coefficient $\binom{n}{i}$, pronounced "n choose i", 
is the number of subsets of $\{1, \ldots, n\}$ which have cardinality $i$.
:::

:::{#prp-binomial-basics}
### properties of the binomial coefficient $\binom{n}{i}$
...
3. In ${\mathbb R}[x]$, every polynomial can be factored into (possibly a real constant and) linear factors $(x-r)$ and rootless quadratic irreducible factors $ax^2+bx + c$ where $b^2-4ac < 0$. These factors are the irreducible polynomials.
4. In ${\mathbb C}[x]$, every polynomial can be factored into (a complex constant and) linear factors $(x-r)$, and these linear factors are the irreducible polynomials.
...
- Let $R = {\mathbb Z}[\sqrt{-5}] \subseteq {\mathbb C}$ be the ring whose elements are of the form $a+b\sqrt{-5}$ for $a, b \in {\mathbb Z}$, with structure inherited from ${\mathbb C}$. Then $R$ is a ring (everyone's first non-UFD domain) and $$2 \cdot 3 = (1-\sqrt{-5}) \cdot (1+\sqrt{-5})$$
...
**Definition:** Let $R$ be a commutative ring and let $I \subseteq R$. We call $I$ an **ideal** if:
...
As for groups, we have a first isomorphism theorem for rings

**Theorem:** Let $f:R \to S$ be a ring homomorphism. Then the image $f(R)$ is a subring of $S$ and is isomorphic to $R/\ker(f)$. Specifically, the map
$$\phi:R/\ker(f) \to \operatorname{im}(f) \enspace \text{ by }  \phi(r+\ker(f)) = f(r)$$
is the isomorphism.

**proof:**
Since $f$ is a homomorphism, $f(R)$ contains $0_S$ and $1_S$. The image is closed under addition and multiplication because $f(x)f(y) = f(xy)$ and $f(x)+f(y) = f(x+y)$. 
Now clearly $f:R \to \operatorname{im}(f)$ sends the elements of $\ker(f)$ to $0_S$, so the universal property of the quotient produces a homomorphism $\phi:R/\ker(f) \to \operatorname{im}(f)$ for which $\phi(r+\ker(f)) = f(r)$. Obviously this $\phi$ is surjective (onto $\operatorname{im}(f)$). To prove that $\phi$ is injective, let us consider $\ker(\phi)$. If $\phi(r+\ker(f)) = 0$ then $f(r)=0$, so $r \in \ker(f)$ and then $r+\ker(f)=\ker(f) = 0_{R/\ker(f)}$ That is, the only element of $\ker(\phi)$ is the zero of its domain ring. It follows that $\phi$ is an isomorphism of rings. 
...
This theorem is often used to create a unique smallest ideal having some desired properties, by intersecting all ideals having that property. Sort of like shrink-wrap. For example, the principal ideal $(a)$ could have been defined as the intersection of the family of every ideal which contains $a$. As long as the property (here "contains $a$") is preserved by intersection, this trick will create
a unique smallest ideal with the property, contained in any other ideal with the property. 


**Definition:** Let $S$ be a subset of $R$. The **ideal generated by $S$,** written $(S)$, is (each of):

i. The intersection of all ideals $I$ of $R$ containing $S$. 
ii. The set of all $R$-linear combinations
$$\{r_1s_1 + r_2s_2 + \ldots + r_ns_n | n \in {\mathbb N}, r_i \in R, s_i \in S\}$$

**proof of equivalence:**
Write $(S)_{\text{outer}}$ for the set described in i, and $(S)_{\text{inner}}$ for the set described in ii. Then certainly $(S)_{\text{outer}}$ is an ideal containing and is the smallest ideal containing $S$. 

The set $(S)_{\text{inner}}$ contains $0$, is closed under addition, and satisfies multiplicative capture, so it is an ideal containing $S$. Any other ideal $I$ containing $S$ must contain the elements of $$(S)_{\text{inner}}$, by capture and closure. It follows that $(S)_{\text{inner}}$ is likewise the unique smallest ideal containing $S$, so the two ideals agree. 


### Unions

The union of ideals is almost never an ideal, because if $a \in I_1$ and $b \in I_2$, there's no reason to believe that $a+b$ is in either ideal, so the union is usually not closed under summation. Of course, if $I \subseteq J$ then their union $I \cup J = J$ is an ideal, but this is isn't creative or useful.  A more creative union is the union of a **chain** of ideals. 

**Definition:** A **chain** $\mathscr C$ of ideals is a family which is totally ordered by the subset relation $\subseteq$.

Note that this is more general than an increasing or decreasing sequence of ideals. A chain may be uncountable and the order structure may be far more complicated than ${\mathbb N}$ or ${\mathbb Z}$. For example, the family of principal ideals $\{(x^r) | r \in (1,\infty)\}$ is an uncountable chain of distinct ideals in the ring of continuous functions on ${\mathbb R}$. 
...
3. The ideal generated by the union $I \cup J$. 

To prove these three equivalent, prove that each set contains $I$ and $J$, is an ideal, and is contained in any other ideal containing both $I$ and $J$. 
...
3. The ideal generated by $\cup \mathscr F$

### Products

Given two ideals $I$, $J$ in a ring $R$, 
the setwise product $\{ij | i \in I, j \in J\}$ satisfies
the capture property, but can fail additive closure, since 
$i_1j_1 + i_2j_2$ need not have a factorization in the 
form $i_3j_3$. As a simple example, if $I = (2,x)$ and 
$J = (3,x)$, then $x \cdot 3 + (-2)\cdot x = x$ cannot be written 
$ij$ for $i \in I$ and $j \in J$. We need to create all sums 
of such products to get additive closure:

**Definition:**
Let $I, J$ be ideals of a ring $R$. The product ideal $IJ$ is defined
to be 
1. The set of all sums of the form 
$$\{i_1j_1 + i_2j_2 + \ldots + j_nj_n | n \in {\mathbb N}, i_k \in I, j_k \in J\}$$
2. The ideal generated by the set $\{ij | i \in I, j \in J\}$
...
1. The prime ideal of ${\mathbb Z}$ are exactly the ideals $(p)$ generated by prime numbers, and the zero ideal.
2. The field ${\mathbb R}$ has only two ideals, and the small one, $(0)$, is prime.
3. A ring $R$ is a domain if and only if the zero ideal is prime.
...
1. The maximal ideals of ${\mathbb Z}$ are, once again, exactly the ideals $(p)$ generated by prime numbers. The zero ideal is not maximal.
2. In function rings, it's common to construct the ideal of all functions $f$ which take the value $0$ at a given fixed point $x \in X$. Often, such an ideal is maximal.
...
This scenario is tailor made for Zorn's Lemma:
...
3. Given a fixed proper ideal $I$, we apply Zorn's Lemma to the poset of all ideals of $R$ which contain $I$, excluding the unit ideal. A maximal element of this poset is a maximal ideal which contains $I$. This demonstrates that every proper ideal is contained in some maximal ideal. 

4. We fix $f \in R$ (not nilpotent) and apply Zorn's Lemma to the poset of ideals which contain neither $f$ nor or any power of $f$. Notice this condition is preserved under chain unions. The result is an ideal $I$ which may not be a maximal ideal, but will always be prime.
...
1. The set of all sequences $a = (a_0, a_1, \ldots, a_n, \ldots)$, where each $a_i \in R$ and all but finitely many are zero, with
...
**Theorem:** Given the structure of $R[x]$, we have a canonical homomorphism $f:R \to R[x]$, and a single element $x \in R[x]$. Given any other homomorphism $g:R \to S$ and chosen element $s \in S$, there is a unique homomorphism $G:R[x] \to S$ making the diagram commute (meaning $Gf = g$) and sending $x$ to $s$.
...
5. More generally, if we have a field $K$ and an important polynomial $p(x)$ with coefficients from $K$, 
we may be interested in whether $p$ has a root $\alpha \in K$ -- that is, an element $\alpha \in K$ so that $p(\alpha)=0$. If there is no root, we may create one by introducing the larger ring $K[x]/(p(x))$, in which $x$ itself is now a root. As long as the ideal $(p(x))$ is maximal, we call the new ring an **extension field** of $K$ by adjoining a root of the polynomial. 
...
5. The **Eisenstein Integers** $Z[\omega]$ where $\omega = \frac{\sqrt{-3}+1}{2}$ (Note $\omega^3=-1$) form a triangular lattice in ${\mathbb C}$. The complex square norm is again a Euclidean function. In this case it satisfies $$|a+bw|^2 = (a+bw)(a+b\overline{w}) = (a+bw)(a+b\overline{w}) = a^2 + b^2 + ab\overline{w}+abw = a^2+b^2 +ab 2\Re(\omega) = a^2+b^2-ab $$



To prove unique factorization in ${\mathbb Z}$, we first prove that every ideal is principal, then use that fact to prove uniqueness of factorizations. The same strategy works more generally if all ideals are principal. Of course even in ${\mathbb Z}$ factorizations differ cosmetically in their ordering and unit multiplication. So for example $30 = 2 \cdot 3 \cdot 5 = 3 \cdot (-5) \cdot (-2)$. Our definition of "unique" needs to allow for such silliness.
...
**Theorem:** Let $K$ be a field. Then $K[x]$ is a Euclidean domain. 

**Proof:** Define Euclidean function $f(p) = deg(p)+1$. Now let $a, b \in K[x]$. Since the leading coefficients cannot be zerodivisors, the degree of the product $ab$ is the sum of the degrees, so $f(a) \leq f(ab)$.  For the second property, we induct on the degree of $a$. If $deg(a) < deg(b)$ we may choose $q=0$ and $r=a$. Next let $b$ have leading term $b_k x^k$ and let $a$ have leading term $a_n x^n$, with $n \geq k$. Let $q_1 = \frac{a_n}{b_k}x^{n-k}$, so that $q_1 b$ has leading term $a_nx^n$. Then $a - q_1b$ has degree less than $n$, so it may be written 
$q_2b+r$, with $deg(r) < k$, by induction. Finally $a = q_1b + q_2b + r = (q_1+q_2)b+ r$ as desired. 

**Note:** The field $K$ itself is also a Euclidean domain. It suffices to choose the function $f:K - \{0\} \to {\mathbb N} - \{0\}$ given by $f(x)=1$. Long division is easy in a field because true division is possible.

## ED $\to$ PID

**Theorem:** (ED $\to$ PID) Every Euclidean Domain is a Principal Ideal Domain

**Proof:** We need only rewrite the proof for ${\mathbb Z}$ in the new context. Let $R$ be a Euclidean domain with function $f$. Let $I$ be an ideal. If $I=(0)$ there's nothing to do. Choose $a \in I$, nonzero, to minimize $f(a)$. Let $b \in I$. Apply long division, so that $b = qa+r$. Now $r = b-qa \in I$. This will contradict the minimality of $a$ if $r$ is nonzero, so $r=0$. It follows that $b$ is divisible by $a$, and since $b$ was arbitrary $I = (a)$.
...
**Proof:** Certainly the units of $R$ have the same reciprocals when regarded as elements of $R[x]$. Conversely if $f(x)$ is a unit in $R[x]$, then $f(x)g(x)=1$ for some $g(x)$. Since $\deg(fg) = \deg(f)+\deg(g)$ (in a domain!) both $f$ and $g$ must be constants in $R$.

**Proposition:** Let $p\in R$ be irreducible. Then the ideal $(p)  = pR[x] \subseteq R[x]$ generated by $p$ in the polynomial ring is prime.

**proof:** Let $f, g \in R[x]$, and suppose $p | fg$. Then every coefficient of $fg$ is divisible by $p$. For contradiction, if neither $f$ nor $g$ is divisible by $p$, then each has a highest-degree term not divisible by $p$. Say $f(x) = p*(h.o.t.)+ a_ix^i + (l.o.t.)$, and $g(x) = p*(h.o.t.)+ b_jx^j + (l.o.t.)$, with $p \nmid a_i$ and $p \nmid b_j$. Then the coefficient of $x^{i+j}$ in $fg$ is a sum of terms, all divisible by $p$ except exactly one. It follows that $fg$ will not be divisible by $p$. It follows that $pR[x]$ is a prime ideal.
...
**Proof: ** In the factorization $f(x) = G(x)H(x)$, both $G$ and $H$ have coefficients which are fractions with denominators from $R$. We multiply both sides of the equation by the product of these denominators to clear them, obtaining $bf = g_1(x)h_1(x)$. Notice that $g_1 = rG$ and $h_1 = sH$ for some $r, s\in R$. Note $b \in R$. By presumption $b$ can be factored into irreducibles $b_1\ldots b_n$. We wish to "cancel" $b_n$ from both sides (and continue), but it doesn't seem to appear on the right. Now $b_nR[x]$ is a prime ideal, and the product $g_1h_1$ is in it, so one of $g_1$ or $h_1$ has coefficients all divisible by $b_n$. Therefore we may divide it by $b_n$ and still have a polynomial in $R[x]$. Continuing this process, we cancel every factor $b_i$ from either $g_1$ or $h_1$, to obtain finally $f(x) = g(x)h(x)$. Now $g_1$ was originally an $R$-multiple of $G$, and is likewise an $R$-multiple of $g$, so $g = k_1G$ for some $k_1 \in K$, and similarly for $h$.
...
It follows immediately that the ideal $f(x)K[x]$ is a prime ideal of $K[x]$, since $K[x]$ is an ED (so PID).  What about the ideal $f(x)R[x]$?  If $g(x)h(x) \in f(x)R[x]$, then this is still true in the larger ring $K[x]$, so one of g or h is in the ideal $f(x)K[x]$.  WLOG $g(x) = f(x)L(x)$, where $L \in K[x]$.
...
1. $S/I$ is a multiplicatively closed set of $R/I$
1. $(S/I)^{-1}(R/I) \cong (S^{-1}R) / (S^{-1}I)$ via a homomorphism $\frac{r+I}{s+I} \mapsto \frac{r}{s} + (S^{-1}I)$

```{mermaid}
%%| fig-cap: "Commutativity of localization and quotient"
%%| fig-alt: ""

graph TD
    %% Accessibility tags
    accTitle: Commutativity of Quotient and Localization
    accDescr: A diagram showing that taking a quotient then localizing is isomorphic to localizing then taking a quotient.
