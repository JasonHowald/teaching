...
Interesting relationships between sets are usually expressed as functions. 
...
1. The letter, symbol, or name representing the function.
2. The domain.
3. The codomain.
4. The rule.
...
More generically, we write "Let $f:D \to R$ by $f(x) = \ldots$ to specify a function called $f$ with domain $D$, codomain $R$, and rule as given.
...
Functions can be different with the same rule and different domains. Functions can have apparently different rules but be the same. Functions can have same domain and image and be technically different with different codomains. Example: max function, average function, eval at 2 function.
